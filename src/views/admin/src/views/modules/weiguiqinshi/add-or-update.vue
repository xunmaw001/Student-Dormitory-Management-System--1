...
export default {
  ...
  methods: {
	...
	this.ruleForm.beizhu = this.ruleForm.beizhu.replace(reg,'../../../nodejsn73cv/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `weiguiqinshi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.weiguiqinshiCrossAddOrUpdateFlag = false;
						 this.parent.search();
